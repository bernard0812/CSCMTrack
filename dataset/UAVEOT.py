...
class UAVEOT(BaseVideoDataset):
    ...
    def _build_sequence_list(self, vid_ids=None, split=None):
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(self.data_specs, "UAVEOT_train_split.txt")
            elif split == 'val':
                file_path = os.path.join(self.data_specs, "UAVEOT_val_split.txt")
            else:
    ...
    def get_event(self, seq_path, frame_id):
        event_path = os.path.join(seq_path, 'CSE', '{:08}.png'.format(frame_id+1))

        return self.image_loader(event_path)
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        return frame_list, event_list, anno_frames, object_meta
