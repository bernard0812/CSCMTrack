...
from .backbone.vit import vit_base_patch16_224, vit_large_patch16_224
from .backbone.vit_ce import vit_large_patch16_224_ce, vit_base_patch16_224_ce
from .fusion.fusion_layer import Fusion_Block
from .fusion.dual_model import Net as dual


class CSCMTrack(nn.Module):
    def __init__(self, transformer, box_head, aux_loss=False, head_type="CORNER", token_len=1, dim=64):
        super().__init__()
    ...
    def forward(self,
                template_image: torch.Tensor = None,
                template_event: torch.Tensor = None,
                search_image: torch.Tensor = None,
        ...
        for i in range(len(template_event)):
            evt = template_event[i]
            template.append(evt)

        for i in range(len(search_event)):
            evt = search_event[i]
            search.append(evt)
...
def build_model(cfg, training=True):
    ...
    pretrained_path = os.path.join(current_dir, '../output/pretrained')
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'vit_base_patch16_224':
        ...
        backbone = vit_base_patch16_224_ce(pretrained, drop_path_rate=cfg.TRAIN.DROP_PATH_RATE,
                                           ...
                                           add_cls_token=cfg.MODEL.BACKBONE.ADD_CLS_TOKEN,
                                           in_chans = cfg.MODEL.BACKBONE.DIM,
                                           )
