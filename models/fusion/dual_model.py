...
class ResidualBlock(torch.nn.Module):
    ...
    def forward(self, data):
        # 主路径
        x = F.elu(self.left_bn1(self.left_conv1(data.x, data.edge_index, data.edge_attr)))
        x = self.left_conv2(x, data.edge_index, data.edge_attr)
        x = self.left_bn2(x)

        # 捷径分支
        shortcut = self.shortcut_conv(data.x, data.edge_index, data.edge_attr)
        shortcut = self.shortcut_bn(shortcut)

        # 残差连接与激活
        data.x = F.elu(x + shortcut)
        return data
...
class ST_Transformer_block(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        temporal_form_out = temporal_form + self.drop_path2(
            self.ls2(self.mlp1(temporal_form)))  # torch.Size([4, 9, 512])
...
class Net(torch.nn.Module):
    def __init__(self, pretrained=None, batchNorm=True, output_layers=None, init_std=0.05,
                 dim=24, clip_len=8, num_heads=6, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=None,
                 drop_path=0., embed_dim=768):
        super(Net, self).__init__()
        self.patch_emd = PatchEmbed(img_size=8, patch_size=4, in_chans=512, embed_dim=embed_dim, norm_layer=None,
                                    flatten=True)
        self.drop = drop
        drop_rate = 0.
    ...
    def forward(self, data_voxel, data_frame):
        ##################################################################################
        B, C, H, W = data_frame.shape  # B 3 8 240 240)
        res_img = F.interpolate(data_frame, size = [self.clip_len, 240, 240], mode='trilinear')

        resnet = self.resnet18_feature_extractor

        feature_extractor = torch.nn.Sequential(*list(resnet.children())[:-2])
        AvgPool2d = nn.AdaptiveAvgPool2d(output_size = (8,8))
        feature_extractor = nn.Sequential(feature_extractor, AvgPool2d)

        img_with_feature = feature_extractor(res_img)  # [b*n,512,8,8]
