...
class Preprocessor(object):
    ...
    def process_event(self, evt_arr: np.ndarray, amask_arr: np.ndarray):
        evt_tensor = torch.tensor(evt_arr).cuda().float().permute((2,0,1)).unsqueeze(dim=0)
        amask_tensor = torch.from_numpy(amask_arr).to(torch.bool).cuda().unsqueeze(dim=0)  # (1,H,W)
        return NestedTensor(evt_tensor, amask_tensor)
