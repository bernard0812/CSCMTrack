...
from .admin.wandb_logger import WandbWriter
...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False):
        ...
        if settings.local_rank in [-1, 0]:
            tensorboard_writer_dir = os.path.join(self.settings.save_dir, "tensorboard")
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            if self.move_data_to_gpu:
                ...
                # data = [tensor.to(self.device) for tensor in data if tensor is isinstance(tensor, torch.Tensor)]
